...
describe('integration boundaries',()=>{
  ...
  it('validates explicit browser messages and caps tab batches',()=>{expect(browserMessageSchema.safeParse({type:'send-tabs',requestId:'1',name:'Research',tabs:[{title:'Docs',url:'https://example.com'}]}).success).toBe(true);expect(browserMessageSchema.safeParse({type:'send-tabs',requestId:'1',name:'Too many',tabs:Array.from({length:101},()=>({title:'x',url:'https://example.com'}))}).success).toBe(false);expect(browserMessageSchema.safeParse({type:'send-page',requestId:'1',page:{title:'bad',url:'file:///tmp/a'},roomId:'room'}).success).toBe(false);});
