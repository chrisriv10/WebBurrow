...
describe('configuration import/export',()=>{
  ...
  it('keeps Config V3 while migrating 1.0 layout-v2 positions',()=>{
    ...
    const migrated=parseConfig(JSON.stringify(legacy));expect(migrated.schemaVersion).toBe(3);expect(migrated.rooms.every(room=>room.layoutVersion===17)).toBe(true);expect(migrated.objects.map(object=>object.id)).toEqual(legacy.objects.map(object=>object.id));
  });
});
