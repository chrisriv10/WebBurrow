...
export function BurrowPortal({room,destination,near,onTravel}:{room:Room;destination:Room;near:boolean;onTravel:()=>void}) {
  ...
  return <group position={ROOM_LAYOUTS[room.template].portal} onClick={onTravel} userData={{interactionId:'__portal'}}>
    <RoundedBox args={[4.7,.35,.55]} position={[0,.2,0]} radius={.12}><meshStandardMaterial color="#24283a" roughness={.78} metalness={.08}/></RoundedBox>
    <RoundedBox args={[.52,4.7,.62]} position={[-2.05,2.45,0]} radius={.14}><meshStandardMaterial color="#23283a" roughness={.8}/></RoundedBox>
    <RoundedBox args={[.52,4.7,.62]} position={[2.05,2.45,0]} radius={.14}><meshStandardMaterial color="#23283a" roughness={.8}/></RoundedBox>
    <group ref={glow} position={[0,2.5,.1]}>
      <mesh><torusGeometry args={[1.55,.2,12,64]}/><meshStandardMaterial color="#3b4557" emissive={room.accent} emissiveIntensity={near?.28:.08} roughness={.78} metalness={.1}/></mesh>
      <group ref={rings}><mesh rotation={[0,0,.55]}><torusGeometry args={[1.22,.022,8,64]}/><meshBasicMaterial color="#b9d9ff" transparent opacity={near ? .62 : .22}/></mesh><mesh rotation={[0,0,-.55]}><torusGeometry args={[.96,.016,8,64]}/><meshBasicMaterial color={room.accent} transparent opacity={near?.48:.2}/></mesh></group>
      <mesh position={[0,0,-.04]}><circleGeometry args={[1.38,64]}/><meshStandardMaterial color="#101827" emissive={room.accent} emissiveIntensity={near?.12:.03} roughness={1}/></mesh>
      <mesh position={[0,0,.02]}><circleGeometry args={[1.08,48]}/><meshBasicMaterial color="#080d18" transparent opacity={.86}/></mesh>
    </group>
