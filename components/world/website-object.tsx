...
function Screen({object,size=[1.35,.78],position=[0,1.35,.36],rotation=[0,0,0],selected,active}:{object:BookmarkObject;size?:[number,number];position?:[number,number,number];rotation?:[number,number,number];selected:boolean;active:boolean}) {
  ...
  return <group position={position} rotation={rotation}>
    <mesh><planeGeometry args={size}/><meshStandardMaterial color="#0b1220" emissive={object.color} emissiveIntensity={active ? 1.05 : selected ? .72 : .42} roughness={.42}/></mesh>
...
function Terminal({object,selected,active}:{object:BookmarkObject;selected:boolean;active:boolean}) {
  return <group><RoundedBox args={[1.75,1.18,.55]} position={[0,1.35,0]} radius={.12} castShadow><meshStandardMaterial color="#171c29" metalness={.28} roughness={.52}/></RoundedBox><Screen object={object} selected={selected} active={active}/><mesh position={[0,.48,0]} castShadow><cylinderGeometry args={[.14,.24,.72,10]}/><meshStandardMaterial color="#252b3a" roughness={.55}/></mesh><RoundedBox args={[1.2,.14,.62]} position={[0,.13,.05]} radius={.05}><meshStandardMaterial color="#202534" roughness={.7}/></RoundedBox><mesh position={[-.55,1.93,.22]}><sphereGeometry args={[.035,8,8]}/><meshBasicMaterial color={active?'#d5fffb':'#69dbc9'}/></mesh></group>;
}

function Television({object,selected,active}:{object:BookmarkObject;selected:boolean;active:boolean}) {
  return <group><RoundedBox args={[2.62,1.62,.46]} position={[0,1.45,0]} radius={.16} castShadow><meshStandardMaterial color="#141925" metalness={.2} roughness={.48}/></RoundedBox><Screen object={object} size={[2.28,1.22]} position={[0,1.46,.24]} selected={selected} active={active}/><RoundedBox args={[1.45,.28,.7]} position={[0,.48,0]} radius={.08} castShadow><meshStandardMaterial color="#252939" roughness={.65}/></RoundedBox><mesh position={[0,.72,.12]}><boxGeometry args={[.1,.45,.12]}/><meshStandardMaterial color="#30364a"/></mesh></group>;
}
...
export function WebsiteObject({object,selected,near,onBeginDrag}:{object:BookmarkObject;selected:boolean;near:boolean;onBeginDrag:(id:string)=>void}) {
  const editMode=useBurrow(s=>s.editMode);const setSelected=useBurrow(s=>s.setSelected);const openSite=useBurrow(s=>s.openSite);const openPulse=useBurrow(s=>s.openPulse);const reduced=useBurrow(s=>s.preferences.reducedEffects);
  const motion=useRef<Group>(null);const [hovered,setHovered]=useState(false);const pulse=useRef(0);const identity=siteIdentity(object);
  ...
  useFrame(({clock},delta)=>{
    ...
    const scale=1+pulse.current*.06+(hovered ? .012 : 0);motion.current.scale.setScalar(scale);
  });
