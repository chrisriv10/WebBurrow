export type WebBurrowPerformanceSnapshot={
  ...
  maxArrivalTaskMs:number;
  reactRenders:Record<string,number>;
  hidden:boolean;
};

const samples:number[]=[];
const counters={drawCalls:0,sceneObjects:0,lights:0,physicsBodies:0,miniBurrowUpdates:0,indexedDbWrites:0,integrationRefreshes:0,maxArrivalTaskMs:0,reactRenders:{} as Record<string,number>,hidden:false};
...
export function recordBrowserArrival(durationMs:number){counters.maxArrivalTaskMs=Math.max(counters.maxArrivalTaskMs,Number(durationMs.toFixed(2)));}
...
export function resetPerformanceWindow(){samples.length=0;}

declare global {interface Window{__WEBBURROW_METRICS__?:()=>WebBurrowPerformanceSnapshot;__WEBBURROW_RESET_METRICS__?:()=>void}}
if(typeof window!=='undefined'){window.__WEBBURROW_METRICS__=performanceSnapshot;window.__WEBBURROW_RESET_METRICS__=resetPerformanceWindow;}
